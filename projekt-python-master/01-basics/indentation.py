...
"""
def debugger_for_c(mistakes):
    """
    Velice užitečný debugger pro jazyk C který určitě ušetří spoustu nervů programátorům
    """
    # zjisti jestli nebyla zadana 0, protoze pak by to tak nejak nefungovalo
    if mistakes == 0:
        print('Nemáš chyby proč to zkoušíš')
    else:
        # pro kazdou chybu napise velice pomocnou radu :)
        for i in range(1, mistakes+1):

            print('Určitě jsi zapomněl/a ; na konci řádku u chyby Č.', i)



print(debugger_for_c.__doc__)
debugger_for_c(5)