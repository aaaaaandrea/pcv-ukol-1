...
def dostatek_spanku(hodiny):
    """
    funkce pro zjisteni jestli clovek dostane dost spanku
    """
    hodiny_int = int(hodiny)
    zbyva_h = 0
    ...
    if 24 > hodiny_int >= 0:
        if hodiny_int > current_h:
            zbyva_h = hodiny_int - current_h
        elif hodiny_int == current_h:
            zbyva_h = 0
        elif hodiny_int < current_h:
            zbyva_h = (24 - current_h) + hodiny_int
        else:
            print('stala se asi nejaka chyba')

        print('Budete mít ' + str(zbyva_h) + ' h spanku pokud pujdete spát ihned')
    else:
        print('Nezadali jste hodiny v rozmezi mezi 0 - 24')


hodina = input('zadejte hodinu kdy chcete vstat (např: 6):')
dostatek_spanku(hodina)
